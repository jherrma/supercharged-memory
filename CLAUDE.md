...
`instructions/MIGRATE-EXISTING-MEMORY.md` is an executable runbook, reached by "migrate my memory" / "import my old memories" or by answering yes to the offer in `SETUP.md` Step 7. It brings ordinary file-based Claude memory — a curated `~/.claude/CLAUDE.md`, `~/.claude/memory/*.md`, project-scoped `~/.claude/projects/<slug>/memory/*.md` — into the database. `scripts/find-existing-memory.py` is its read-only probe: dependency-free (no DB, no Ollama) so Step 7 can run it before either is proven working, it reports per file a `kind` (`memory` / `index` / `claude`), a `scope` (`global` / `project`) and a char count against `MAX_TEXT`, and every count it reports covers `kind: memory` only — an index is a pointer list and a `CLAUDE.md` body is instructions, so neither is offered as memory to import.

Classification runs in **subagents** on the same contract as the sleep runbooks; the orchestrator never reads the corpus in bulk (a real corpus measured 233 files / 634k chars). Every imported row is tagged `source='migration'` with a `file_reference` back to its file — the resume position for an interrupted import, and the audit trail afterwards. Two things the runbook is explicit about, because both are easy to get wrong: the import is **additive, not reversible** (no bulk undo of the imported rows — the untouched source files are always the way back, M2's pre-import dump restores a database that already held rows, and on the common `EMPTY` case there is no dump at all because nothing is at risk), and D3 compaction in the follow-up deep-sleep phases writes `--source deep-sleep` with no `--file-reference`, so the manifest is exported to a file *before* D3 or the provenance is gone.
